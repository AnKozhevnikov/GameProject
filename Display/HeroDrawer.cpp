...
HeroDrawer::HeroDrawer(PHero phero, int x_, int y_) {
    ...
    SetHp(phero->get_hp(), phero->get_maxHp());
    SetAttention(phero->get_attention());
    SetDmg(phero->get_dmg());
    SetInitiative(phero->get_initiative());
    SetName(phero->get_name());
}
...
void HeroDrawer::SetChoice(bool state, short clr) const {
    ...
    if (state) {
        display.DrawSprite(std::vector<std::vector<unsigned>>(1, std::vector<unsigned>(BattleViewConstants::HeroLength,
                                                                                       '=' |
                                                                                       manager.CreateColorPair(clr,
                                                                                                               -1) |
                                                                                       A_BLINK)), x,
                           y + BattleViewConstants::HeroHeight + 1);
    } else {
        display.DrawSprite(std::vector<std::vector<unsigned>>(1, std::vector<unsigned>(BattleViewConstants::HeroLength,
                                                                                       ' ' | manager.CreateColorPair(-1,
                                                                                                                     -1))),
                           x, y + BattleViewConstants::HeroHeight + 1);
    }
}
...
void HeroDrawer::SetHp(int cur_hp, int max_hp) {
    ...
    display.DrawText(std::string(BattleViewConstants::HeroLength, ' '), x,
                     y + BattleViewConstants::HeroHeight + 2); //Clear old hp
...
void HeroDrawer::SetDmg(int value) {
    ...
    display.DrawText(std::string(BattleViewConstants::HeroLength / 2, ' '), x,
                     y + BattleViewConstants::HeroHeight + 3); //Clear old dmg
...
void HeroDrawer::SetAttention(int value) {
    ...
    display.DrawText(std::string(BattleViewConstants::HeroLength / 2, ' '), x + BattleViewConstants::HeroLength / 2,
                     y + BattleViewConstants::HeroHeight + 3); //Clear old attention
    display.DrawText(attention, x + BattleViewConstants::HeroLength / 2,
                     y + BattleViewConstants::HeroHeight + 3); //Draw new attention
}
...
void HeroDrawer::SetInitiative(int value) {
    ...
    display.DrawText(std::string(BattleViewConstants::HeroLength, ' '), x,
                     y + BattleViewConstants::HeroHeight + 4); //Clear old initiative
...
HeroDrawer::~HeroDrawer() {
    ...
    display.DrawSprite(std::vector<std::vector<unsigned>>(BattleViewConstants::HeroHeight + 5,
                                                          std::vector<unsigned>(BattleViewConstants::HeroLength, ' ')),
                       x, y - 1);
}
...
void HeroDrawer::ApplyEffect(VisualEffect effect, bool state) {
    ...
    switch (effect) {
        ...
        case VisualEffect::DAMAGE:
            ...
            break;
        case VisualEffect::HEAL_STATUS_EFFECTS:
            this->ApplyHealStatusEffects(state);
            break;
...
void HeroDrawer::ApplyDead(int state) {
    ...
    if (is_running[VisualEffect::DEAD]) {
        std::vector<std::vector<unsigned>> EmptyWindow(BattleViewConstants::HeroHeight,
                                                       std::vector<unsigned>(BattleViewConstants::HeroLength,
                                                                             ' ' | manager.CreateColorPair(-1, -1)));
...
bool HeroDrawer::UpdateAnimations() {
    ...
    std::vector<std::vector<unsigned>> EmptyWindow(BattleViewConstants::HeroHeight,
                                                   std::vector<unsigned>(BattleViewConstants::HeroLength,
                                                                         ' ' | manager.CreateColorPair(-1, -1)));
    layers = std::vector<std::vector<unsigned>>(BattleViewConstants::HeroHeight,
                                                std::vector<unsigned>(BattleViewConstants::HeroLength, 0));
    addLayer(EmptyWindow);
    addLayer(my_sprite);
    this->ApplyBurn(-1);
    this->ApplyStun(-1);
    int result = this->ApplyHeal(false) +
            this->ApplyDamage(false) +
            this->ApplyHealStatusEffects(false);
    this->ApplyDead(-1);
...
bool HeroDrawer::ApplyHealStatusEffects(bool state) {
    Display display;
    ColorManager manager;
    if (state || is_running[VisualEffect::HEAL_STATUS_EFFECTS]) {
        if (!is_running[VisualEffect::HEAL_STATUS_EFFECTS]) {
            is_running[VisualEffect::HEAL_STATUS_EFFECTS] = true;
            start[VisualEffect::HEAL_STATUS_EFFECTS] = std::chrono::system_clock::now();
        }
        auto now = std::chrono::system_clock::now();
        auto elapsed = std::chrono::duration_cast<milliseconds>(now - start[VisualEffect::HEAL_STATUS_EFFECTS]).count();

        if (elapsed > 1200) {
            is_running[VisualEffect::HEAL_STATUS_EFFECTS] = false;
            return false;
        } else if (elapsed > 900) {
            addLayer(Drawer::getSprite("status_heal3", '+'));
        } else if (elapsed > 600) {
            addLayer(Drawer::getSprite("status_heal2", '+'));
        } else if (elapsed > 300) {
            addLayer(Drawer::getSprite("status_heal1", '+'));
        }
        return true;
    }
    return is_running[VisualEffect::HEAL_STATUS_EFFECTS];
}
