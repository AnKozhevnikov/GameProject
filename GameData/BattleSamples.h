...
namespace SampleAbilities {
    ...
    static Ability roar = Ability("Roar", "Heal one ally", Drawer::getSprite("abil_heal"), true, false, 0, 1, 0, 0, 0);
...
namespace SampleItems {
    static Item gold = Item("Gold", 10000, Drawer::getSprite("mini_coin"), "Gold is used to buy items and abilities", SampleAbilities::voidAbility);
    static Item healthPotions = Item("Health Potions", 100, Drawer::getSprite("potion"), "Health Potions are used to heal your heroes", SampleAbilities::healPotion);
    static Item bombs = Item("Bombs", 100, Drawer::getSprite("bomb"), "Bombs are used to deal damage to enemies", SampleAbilities::bomb);
    static Item fireBombs = Item("Fire Bombs", 100, Drawer::getSprite("molotov"), "Fire Bombs are used to put enemies on fire", SampleAbilities::fireBomb);
    static Item stunBombs = Item("Stun Bombs", 100, Drawer::getSprite("flash"), "Stun Bombs are used to stun enemies", SampleAbilities::stunBomb);
}
...
namespace SampleSides {
    ...
    static std::vector<Hero> evil0 = std::vector<Hero>({
        SampleHeroes::farmer,
        SampleHeroes::mini_ghost,
        SampleHeroes::mini_sheep,
        SampleHeroes::fly
    });

    static std::vector<Hero> evil1 = std::vector<Hero>({
    ...
    static std::vector<std::vector<Hero>> bossTeams = std::vector<std::vector<Hero>>({
        std::vector<Hero>({SampleHeroes::farmer, SampleHeroes::megaWarrior, SampleHeroes::farmer}),
        std::vector<Hero>({SampleHeroes::mini_ghost, SampleHeroes::ghost, SampleHeroes::mini_ghost}),
        std::vector<Hero>({SampleHeroes::mini_sheep, SampleHeroes::sheep, SampleHeroes::mini_sheep}),
        std::vector<Hero>({SampleHeroes::fly, SampleHeroes::frog, SampleHeroes::fly})
    });
}